...
const INDEX_HTML = `
    ...
    <html lang="ru">
        <head>
            <meta charset="utf-8">
            <title>Последние новости с pstgu.ru</title>
			<style>
				a:hover {
        ...
        <body>
                {{range .}}
                    <a href="https://pstgu.ru{{.Ref}}" style="color: #333 !important;font-size: 24px;font-weight: 700;text-decoration: none;line-height: 34px;" target = "_blank">
                    	{{.Title}}
                    </a>
                    <br/>
                {{end}}
		 <a href="https://pstgu.ru/news">
			<h1 style="color: #333 !important">Все новости</h1></a>
        </body>
    </html>
    `
